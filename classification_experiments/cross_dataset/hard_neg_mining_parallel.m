...
while 1
    ...
    while length(dir([lock_dir, 'lock_negmine_file_*'])) < length(neg_files)
        ...
        neg = neg.class_feat;
        [~, ~, dec_values] = predict(double(-ones(size(neg,1),1)), sparse(double(neg)), model, '-b 1');
        dec_values = dec_values(:, model.Label==1);
        [ap, prec, rec] = myAP(dec_values,double(-ones(size(neg,1),1)), 1); 
        
        hard_idx = find(dec_values > 0.5);
        ...
        fname = sprintf('%s/hard_idx_file_%d_iter_%d.mat', lock_dir, f, mine_iter);
...
%     
    ...
    if exist('i_am_server', 'var') && i_am_server == true
        ...
        for f = 1 : length(neg_files)
            neg = load(neg_files{f});
            hfile = sprintf('%s/hard_idx_file_%d_iter_%d.mat', lock_dir, f, mine_iter);
            hidx = load(hfile);
        ...
        for f = 1 : length(neg_files)
            ...
            [~, ~, dec_values] = predict(double(-ones(numel(neg_cache{f}), 1)), sparse(double(hard_neg_feat{f})), model, '-b 1');
            dec_values = dec_values(:, model.Label==1);
            [ap, prec, rec] = myAP(dec_values,double(-ones(numel(neg_cache{f}), 1)), 1); 
            
            hard_idx = find(dec_values > 0.5);
            neg_cache{f} = neg_cache{f}(hard_idx);
        end
        mine_iter = mine_iter + 1;
        save(neg_cache_file, 'neg_cache', 'new_cnt', 'mine_iter');
